			...
			if (!$offset && preg_match("~^$space*+DELIMITER\\s+(\\S+)~i", $query, $match)) {
				...
				if (!$found && $fp && !feof($fp)) {
					...
					if ($found && rtrim($found) != $delimiter) { // find matching quote or comment end
						...
						$print .= generate_linksbar(["<a href='#' class='copy-to-clipboard'>Copy to clipboard</a>"]);
							...
							if ($connection->error) {
								...
								if (is_object($result)) {
									...
									if (!$_POST["only_errors"]) {
										...
										echo "<p>" . ($num_rows ? ($limit && $num_rows > $limit ? sprintf('%d / ', $limit) : "") . sprintf('%d row(s)', $num_rows) : "");
										echo $time;
...
if (!isset($_GET["import"])) {
	...
	if ($importServerPath) {
		...
		echo 'Webserver file <code>' . h($importServerPath) . "$gz</code>";
...
if (!isset($_GET["import"]) && $history) {
	...
	for ($val = end($history); $val; $val = prev($history)) { // not array_reverse() to save memory
		...
		echo '<a href="' . h(ME . "sql=&history=$key") . '" class="edit" title="Edit">Edit</a>'
	...
	echo "<a href='" . h(ME . "sql=&history=all") . "' class='edit-all' title='Edit all'>Edit all</a>\n";
	echo "</div></fieldset>\n";
}
?>
</form>
